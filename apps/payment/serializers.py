...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'amount', 'currency', 'stauts', 'payment_method', 'description',
            'is_successful', 'is_pending', 'can_be_refunded',
            'created_at', 'updated_at', 'processes_at'
        ]
        read_only_fields = [
    ...
    def get_subscription_info(self, obj):
        """Возвращает информаию о подписке"""
        if obj.subscription:
            return {
                'id': obj.subscription.id,
                'plan.name': obj.subscription.plan_name,
...
class PaymentCreateSerialiazer(serializers.Serializer):
    ...
    cencel_url = serializers.URLField(required=False)
    ...
    def validate(self, attrs):
        ...
        if hasattr(user, 'subscription') and user.subscription.is_activve:
            raise serializers.ValidationError({
                'non_failed_errors': [
...
class PaymentAttemptSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'stripe_charge_id', 'stauts', 'error_message',
            'metadata', 'created_at'
        ]
...
class RefundSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'status', 'created_at', 'created_by', 'processed_at'
        ]
    ...
    def get_created_by_info(self, obj):
        ...
        if obj.created_by:
            return {
                'id': obj.creted_by.id,
                'username': obj.creeated_by.username
            }
        return None
    ...
    def validate(self, attrs):
        ...
        if payment_id:
            ...
            total_refunded = payment.refunds.filter(
                status='succeeded'
            ).aggregate(total=models.Sum('amount'))['totla'] or Decimal('0')

            if attrs['amount'] > (payment.amount - total_refunded):
                raise serializers.ValidationError(
                    'Refund amount exceeds remainig payment amount.'
                )
        return attrs
...
class RefundCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_amount(self, value):
        """Валидация суммы возврата"""
        if value <= 0:
            raise serializers.ValidationError('Refund amount must be postive.')
        return value
