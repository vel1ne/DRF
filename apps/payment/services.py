...
class StripeService:
                ...
                line_items=[{
                    'price_data': {
                        ...
                        'product_data': {
                            'name': (
                                'Subscription - '
                                f'{payment.subscription.plan.name}'
                            ),
    ...
    def create_payment_intent(payment: Payment) -> Optional[str]:
        """Создает Payment Intent in Stripe"""
        try:
            intent = stripe.PaymentIntent.create(
                amount=int(payment.amount * 100),  # В центах
                currency=payment.stripe_customer_id,
                metadata={
                    'payment_id': payment.id,
                    'user_id': payment.user.id,
                    'subscription_id': (
    ...
    @staticmethod
    def retrive_sesson(session_id: str) -> Optional[Dict]:
...
class PaymentService:
    ...
    def process_successful_payment(payment: Payment) -> bool:
        ...
        try:
            ...
            logger.error(
                f'Error processing successful payment {payment.id}: {e}'
            )
            return False
...
class WebhookService:
    ...
    def _handle_payment_failed(event_data: Dict) -> bool:
        ...
        try:
            ...
            return PaymentService.process_successful_payment(
    ...
    def _handle_dispute_created(event_data: Dict) -> bool:
        """Обрабатывает сщздание диспута"""
        try:
            dispute = event_data['data']['object']
            charge_id = dispute.get('charge')

            # Здесь можно добавить логику обработки диспутов
