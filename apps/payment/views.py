...
def create_checkout_session(request):
    """Создаёт Stripe Checkout сессию для оплаты подписки"""
    serializer = PaymentCreateSerialiazer(
        ...
        try:
            with transaction.atomic():
                ...
                payment, subscription = (
                    PaymentService.create_subscription_payment(
                        request.user, plan
                    )
                )
                ...
                success_url = serializer.validated_data.get(
                    'success_url',
                    f'{settings.FRONTEND_URL}/payment/suceess?session_id={{CHECKOUT_SESSION_ID}}'
                )
...
def payment_status(request, payment_id):
    ...
    try:
        ...
        if payment.stripe_session_id and payment.status in [
            ...
            session_info = StripeService.retrive_sesson(
                payment.stripe_session_id
            )

            if session_info:
                if session_info['stastus'] == 'complete':
                    PaymentService.process_successful_payment(payment)
                elif session_info['stastus'] == 'failed':
        ...
        if payment.is_successful and payment.subscription:
            response_data['subscription_activated'] = (
...
class RefundListView(generics.ListAPIView):
    """Список возвратов для администратора"""
...
def create_refund(request, payment_id):
    """Создаёт возват для платежа"""
    try:
        payment = get_object_or_404(Payment, id=payment_id)

        if not payment.can_be_refunded:
            return Response({
                'error': 'This payment cennot be refunded'
            }, status=status.HTTP_400_BAD_REQUEST)
...
def payment_analytics(request):
    """Аналитика по платжам для администраторов"""
    ...
    montly_payments = Payment.objects.filter(
        created_at__gte=last_month,
        status='succeeded'
    )
    montly_revenue = montly_payments.aggregate(
        total=Sum('amount')
    )['total'] or 0
    montly_count = montly_payments.count()
    ...
    actiive_subsriptions = Payment.objects.filter(
        status='suceeded',
    ...
    return Response({
        ...
        'montly_revenue': float(montly_revenue),
        'montly_payments': montly_count,
        'average_paymetn': float(avg_payment),
        'actiive_subsriptions': actiive_subsriptions,
        'period': {
...
def retry_payment(request, payment_id):
    ...
    try:
        ...
        # Создаём новую сессию для повторной оплаты
