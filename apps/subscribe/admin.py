...
class SubscriptionAdmin(admin.ModelAdmin):
    ...
    readonly_fields = (
        'created_at', 'updated_at', 'is_active', 'days_remainig'
    )
    raw_id_fields = ('user',)
    ...
    activate_subscriptions.short_description = (
        'Activete selected subscriptions'
    )
...
class PinnedPostAdmin(admin.ModelAdmin):
    ...
    def subscription_status(self, obj):
        ...
        if (
            hasattr(obj.user, 'subscription')
            and obj.user.subscription.is_active
        ):
            return format_html('<span style="color: green;">/ Active</span>')
        else:
            return format_html('<span style="color: red;">/ Inactive</span>')
...
class SubscriptionHistoryAdmin(admin.ModelAdmin):
    ...
    search_fields = ('subscription__user-_username', 'description')
    readonly_fields = (
        'subscription', 'action', 'description', 'metadata', 'created_at'
    )

    def subscription_link(self, obj):
        """Ссылка на подписку"""
        url = reverse(
            'admin:subscribe_subscription_change',
